...
resource "aws_cloudfront_cache_policy" "backend_api" {
  count = local.api_origin_enabled ? 1 : 0

  name        = "${local.name}-backend-api-cache"
  comment     = "Disable caching for ${local.name} backend API requests"
  default_ttl = 0
  max_ttl     = 0
  min_ttl     = 0
...
resource "aws_cloudfront_distribution" "frontend" {
  ...
  dynamic "ordered_cache_behavior" {
    ...
    content {
      ...
      cache_policy_id          = aws_cloudfront_cache_policy.backend_api[0].id
      compress                 = true
