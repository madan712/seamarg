...
`seamarg` is a monorepo for an AI-assisted maritime career/compliance platform for seafarers. Four deployable parts:
...
npm workspaces are `frontend` and `lambda`; the backend is Gradle-only. Import the repo root into IntelliJ as a Gradle project.
